...
var rootCmd = &cobra.Command{
	Use:   "linkding-to-markdown",
	Short: "Fetch bookmarks from Linkding and export them to Markdown",
	Long: `linkding-to-markdown fetches bookmarks from a Linkding instance over a given
timespan and produces a formatted Markdown document.

You can filter bookmarks by date range, search query, and customize the output
format. The tool supports both configuration files and command-line flags.`,
...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is ./linkding-to-markdown.yaml)")
...
func initConfig() {
	if cfgFile != "" {
		...
		viper.SetConfigName("linkding-to-markdown")
	}
